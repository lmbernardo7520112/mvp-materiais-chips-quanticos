...
def test_c2_module_imports():
    """GREEN esperado: importar modulo sem erro."""
...
def test_interface_sheet_source_dataclass_contract():
    ...
    source = InterfaceSheetSource(
        sigma_c_per_m2=1e-4, surface_dimension="1D", geometry_label="top_gate"
    )
...
def test_interface_sheet_source_requires_sigma_units():
    ...
    with pytest.raises(ValueError, match="Units must be C/m²"):
        InterfaceSheetSource(
            sigma_c_per_m2=1e-4, surface_dimension="1D", geometry_label="top_gate", units="C/cm2"
        )
...
def test_interface_sheet_source_preserves_charge_sign():
    ...
    source_pos = InterfaceSheetSource(
        sigma_c_per_m2=1e-4, surface_dimension="1D", geometry_label="top_gate"
    )
    source_neg = InterfaceSheetSource(
        sigma_c_per_m2=-1e-4, surface_dimension="1D", geometry_label="top_gate"
    )
...
def test_conservative_regularization_requires_l_reg():
    ...
    with pytest.raises(TypeError):
        ConservativeVolumeRegularization(sigma_c_per_m2=1e-4)  # type: ignore
...
def test_depth_prior_requires_trap_family():
    ...
    valid = DepthPriorMetadata(
        trap_family="border",
        source="Test",
        source_role="test",
        material_stack="Si",
        interface_or_region="Si",
        depth_or_distribution_parameter="1nm",
        length_units="nm",
        transferability_note="None",
...
def test_e1_depth_prior_requires_metadata():
    ...
    required_fields = [
        ...
        "material_stack",
        "interface_or_region",
        "depth_or_distribution_parameter",
        "length_units",
...
def test_no_quantum_confinement_solver_import():
    ...
    with contextlib.suppress(ModuleNotFoundError):
        import mvp_quantum_materials.c2_charge_mapping  # noqa: F401
