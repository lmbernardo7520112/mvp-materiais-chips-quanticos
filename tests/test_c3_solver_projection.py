...
def test_c3_projection_module_imports():
    ...
    assert hasattr(c3, "GridGeometry")
    assert hasattr(c3, "BoundaryConditionMetadata")
    assert hasattr(c3, "ProjectedC3Source")
    assert hasattr(c3, "project_c2_source_to_grid")
    assert hasattr(c3, "validate_projected_charge_conservation")
...
def test_projection_requires_cell_area_or_volume():
    ...
    with pytest.raises(ValueError, match="cell area or volume must be defined"):
        c3.project_c2_source_to_grid(
            source="dummy", domain="dummy", grid_spacing=1.0, cell_area=None, cell_volume=None
        )
...
def test_projection_preserves_total_charge_uniform_case():
    ...
    c3 = importlib.import_module("mvp_quantum_materials.c3_solver_projection")
    result = c3.project_c2_source_to_grid(
        source={"total_charge": 1e-18}, domain="dummy", grid_spacing=1.0, cell_area=1.0
    )
...
def test_projection_preserves_total_charge_discrete_case():
    ...
    result = c3.project_c2_source_to_grid(
        source={"total_charge": source_charge}, domain="dummy", grid_spacing=1.0, cell_area=1.0
    )
...
def test_projection_preserves_charge_sign():
    ...
    c3 = importlib.import_module("mvp_quantum_materials.c3_solver_projection")
    result = c3.project_c2_source_to_grid(
        source={"total_charge": -1.6e-19}, domain="dummy", grid_spacing=1.0, cell_area=1.0
    )
...
def test_projection_preserves_c2_metadata():
    ...
    result = c3.project_c2_source_to_grid(
        source={"total_charge": 1e-18, "metadata": metadata},
        domain="dummy",
        grid_spacing=1.0,
        cell_area=1.0,
    )
...
def test_projection_requires_boundary_condition_metadata():
    ...
    with pytest.raises(ValueError, match="boundary condition metadata is required"):
        c3.project_c2_source_to_grid(
            source="dummy", domain="dummy", grid_spacing=1.0, cell_area=1.0, bc_metadata=None
        )
...
def test_projection_requires_geometry_label():
    ...
    with pytest.raises(ValueError, match="geometry label is required"):
        c3.project_c2_source_to_grid(
            source="dummy", domain="dummy", grid_spacing=1.0, cell_area=1.0, geometry_label=None
        )
...
def test_no_poisson_solver_import_in_projection_module():
    ...
    c3 = importlib.import_module("mvp_quantum_materials.c3_solver_projection")
    source = inspect.getsource(c3)

    lines = source.split("\n")
    for line in lines:
        line = line.strip()
        if line.startswith("import ") or line.startswith("from "):
            assert "poisson_solver_2d" not in line
        assert "solve_poisson" not in line
        assert "poisson runtime" not in line.lower()
...
def test_no_ai_for_science_imports():
    ...
    for line in lines:
        line = line.strip()
        if line.startswith("import ") or line.startswith("from "):
            for f in ["torch", "tensorflow", "jax", "sklearn", "scikit"]:
                assert f not in line
        assert "PINN" not in line
        assert "surrogate" not in line

    for ml_pkg in ["torch", "tensorflow", "jax", "sklearn"]:
...
def test_projection_is_deterministic():
    ...
    c3 = importlib.import_module("mvp_quantum_materials.c3_solver_projection")
    res1 = c3.project_c2_source_to_grid(
        source={"total_charge": 1e-18}, domain="dummy", grid_spacing=1.0, cell_area=1.0
    )
    res2 = c3.project_c2_source_to_grid(
        source={"total_charge": 1e-18}, domain="dummy", grid_spacing=1.0, cell_area=1.0
    )
